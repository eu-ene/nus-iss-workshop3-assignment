...
class TravelPlannerOrchestrator:
    ...
    def plan(self,
        ...
        ranked_flights = rank_items_via_llm("flight", top_flights, {**context, "role_budget": allocation["flight"]}, top_k=3) if top_flights else []
        ranked_hotels = rank_items_via_llm("hotel", top_hotels, {**context, "role_budget": allocation["hotel"]}, top_k=3) if top_hotels else []
        ranked_restaurants = rank_items_via_llm("restaurant", top_restaurants, {**context, "role_budget": allocation["restaurant"], "nights": nights}, top_k=6) if top_restaurants else []
        ...
        for r in ranked_restaurants:
            price = r.get("estimated_price", 0) or r.get("price", 0)
            if price == 0:
        ...
        restaurants_cost = sum(r.get("estimated_price", 0) for r in chosen_restaurants)
        ...
        plan_summary = summarize_plan_via_llm(plan)
